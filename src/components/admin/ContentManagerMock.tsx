...
import { Plus, Edit, Trash2, Save, X, Play, FileText, ExternalLink } from 'lucide-react';
...
const ContentManagerMock: React.FC<ContentManagerProps> = ({ type }) => {
  ...
  const mockCourses = [
    {
      ...
      description: 'Apprendre les bases des équations du premier degré',
      content: 'Les équations sont des égalités contenant des inconnues...',
      level: 'débutant',
      media_type: 'texte',
      media_url: '',
      subject_id: '1',
    ...
    },
    {
      ...
      content: 'La géométrie plane étudie les figures dans un plan...',
      level: 'intermédiaire',
      media_type: 'texte',
      media_url: '',
      subject_id: '1',
    ...
    },
    {
      ...
      description: 'Introduction à la chimie des composés carbonés',
      content: 'La chimie organique étudie les composés contenant du carbone...',
      level: 'avancé',
      media_type: 'texte',
      media_url: '',
      subject_id: '2',
      order_index: 1,
      subject: { name: 'Sciences' },
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString()
    }
  ];
    ...
    try {
      ...
      if (editingItem) {
        ...
        const newItem = {
          ...
          created_at: new Date().toISOString(),
          updated_at: new Date().toISOString(),
          subject: type === 'courses' ? subjects.find(s => s.id === formData.subject_id) : undefined
        };
      ...
      setShowForm(false);
      setEditingItem(null);
      alert(type === 'subjects' ? 'Matière sauvegardée avec succès !' : 'Cours sauvegardé avec succès !');
    } catch (error) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      alert(type === 'subjects' ? 'Matière supprimée avec succès !' : 'Cours supprimé avec succès !');
    } catch (error) {
      ...
      return (
        <div className="space-y-4">
          ...
          {formData.media_type !== 'texte' && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                URL du média {formData.media_type === 'video' ? '(YouTube, Vimeo...)' : '(PDF, DOC, lien)'}
              </label>
              <input
                type="url"
                ...
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                placeholder={formData.media_type === 'video' ? 'https://youtu.be/...' : 'https://.../document.pdf'}
              />
              <p className="text-xs text-gray-500 mt-1">Collez un lien valide vers votre {formData.media_type}.</p>
              ...
              {formData.media_type === 'document' && formData.media_url && (
                <div className="mt-3 p-3 border border-gray-200 rounded-lg flex items-center justify-between bg-gray-50">
                  <div className="flex items-center space-x-3">
                    ...
                    <span className="text-sm text-gray-700 truncate max-w-[320px]">{formData.media_url}</span>
                    {isPdf(formData.media_url) && (
                      <span className="text-xs px-2 py-0.5 bg-red-100 text-red-700 rounded">PDF</span>
                    )}
                  </div>
                  <a href={formData.media_url} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline text-sm flex items-center">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap max-w-xs">
                        {item.media_url ? (
                          item.media_type === 'video' && getYouTubeEmbedUrl(item.media_url) ? (
                            ...
                            <a href={item.media_url} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline truncate inline-block max-w-xs">
                              {item.media_url}
                            </a>
                          )
                        ) : (
...
export default ContentManagerMock;
